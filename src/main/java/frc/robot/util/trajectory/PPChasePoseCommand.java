...
public class PPChasePoseCommand extends Command implements Logged {
    ...
    /**
     * Constructs a command to follow a moving target pose. Uses PathPlanner
     * trajectories when the target is more than 0.2 m away.
     * 
     * @param targetPose                       A Supplier for the target pose.
     * @param pose                             A Supplier for the current pose.
     * @param driveController                  The PPHolonomicDriveController to
     *                                         use.
    ...
    public void execute() {
        ...
        // controller to hold the pose.
            ...
            targetChassisSpeeds = new ChassisSpeeds(
                    ...
                    m_thetaController.calculate(pose.getRotation().getRadians(), target.getRotation().getRadians()));
